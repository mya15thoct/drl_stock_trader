...
1. File này quản lý bộ nhớ Replay Buffer, giúp lưu trữ và lấy mẫu trải nghiệm trong DRL.
2. Hỗ trợ lấy mẫu ngẫu nhiên và Prioritized Experience Replay (PER) để tối ưu quá trình huấn luyện.
3. Cập nhật và lưu trọng số ưu tiên dựa trên sai số TD, cải thiện hiệu suất học.
4. Cho phép lưu và tải lại bộ nhớ, giúp tiếp tục huấn luyện mà không mất dữ liệu.
'''
import os
import math
import torch as th
...
class ReplayBuffer:  # for off-policy
    def __init__(self,
        ...
        #PER (Prioritized Experience Replay) giúp ưu tiên lưu và lấy mẫu các trải nghiệm quan trọng hơn.
    ...
    def update(self, items: Tuple[TEN, ...]):
        '''
        phương thức update() của Replay Buffer trong Reinforcement Learning (RL), 
        dùng để cập nhật buffer với các trải nghiệm mới từ môi trường. 
        '''
    ...
    def sample(self, batch_size: int) -> Tuple[TEN, TEN, TEN, TEN, TEN, TEN]:
        '''
         Lấy mẫu ngẫu nhiên (dùng cho Replay Buffer thông thường).
        '''
    ...
    def sample_for_per(self, batch_size: int) -> Tuple[TEN, TEN, TEN, TEN, TEN, TEN, TEN, TEN]:
        """
        dùng để lấy mẫu theo Prioritized Experience Replay (PER): ưu tiên các trải nghiệm quan trọng hơn.
        """
        # Đảm bảo batch_size chia hết cho num_seqs
        original_batch_size = batch_size
        if batch_size % self.num_seqs != 0:
            batch_size = (batch_size // self.num_seqs) * self.num_seqs
            if batch_size == 0:
                batch_size = self.num_seqs
        
        # Kiểm tra xem cur_size có đủ lớn không
        sample_len = max(1, self.cur_size - 1)  # Đảm bảo sample_len >= 1
        ...
        for env_i in range(self.num_seqs):
            ...
            _is_indices, _is_weights = sum_tree.important_sampling(sub_batch_size, beg, end, self.per_beta)
            
            # Đảm bảo indices không vượt quá giới hạn
            _is_indices = torch.clamp(_is_indices, 0, self.cur_size - 1)
            
            is_indices.append(_is_indices + sub_batch_size * env_i)
            is_weights.append(_is_weights)

        is_indices = torch.cat(is_indices).to(self.device)
        is_weights = torch.cat(is_weights).to(self.device)

        # Đảm bảo is_indices nằm trong phạm vi hợp lệ
        is_indices = torch.clamp(is_indices, 0, self.cur_size * self.num_seqs - 1)
        
        self.ids0 = ids0 = torch.fmod(is_indices, self.cur_size)  # is_indices % sample_len
        self.ids1 = ids1 = torch.div(is_indices, self.cur_size, rounding_mode='floor')  # is_indices // sample_len
    ...
    def td_error_update_for_per(self, is_indices: TEN, td_error: TEN):
        # Vẫn giữ các tính toán trên GPU
        prob = td_error.clamp(1e-8, 10).pow(self.per_alpha)
        
        # Kiểm tra kích thước batch
        batch_size = td_error.shape[0]
        if batch_size % self.num_seqs != 0:
            # Điều chỉnh batch_size để chia hết cho num_seqs
            new_batch_size = (batch_size // self.num_seqs) * self.num_seqs
            is_indices = is_indices[:new_batch_size]
            prob = prob[:new_batch_size]
            batch_size = new_batch_size
        
        sub_batch_size = batch_size // self.num_seqs
        
        # Kiểm tra sub_batch_size
        if sub_batch_size <= 0:
            return  # Bỏ qua nếu sub_batch_size không hợp lệ
        
        # Chỉ chuyển sang CPU khi cần thiết cho sum_tree.update_ids
        for env_i in range(self.num_seqs):
            slice_i = env_i * sub_batch_size
            slice_j = slice_i + sub_batch_size
            
            # Kiểm tra giới hạn
            if slice_j > batch_size:
                slice_j = batch_size
                
            # Trích xuất các chỉ số và xác suất cho môi trường hiện tại
            indices_slice = is_indices[slice_i:slice_j]
            prob_slice = prob[slice_i:slice_j]
            
            # Kiểm tra giá trị indices trước khi chuyển sang CPU
            max_idx = self.cur_size - 1  # Giới hạn hợp lệ của indices
            
            # Lọc các indices không hợp lệ trên GPU
            valid_mask = (indices_slice >= 0) & (indices_slice <= max_idx)
            
            if not valid_mask.all():
                # Chỉ lấy các indices hợp lệ
                valid_indices = torch.nonzero(valid_mask).squeeze()
                if valid_indices.numel() == 0:
                    continue  # Bỏ qua nếu không có indices hợp lệ
                    
                indices_slice = torch.index_select(indices_slice, 0, valid_indices)
                prob_slice = torch.index_select(prob_slice, 0, valid_indices)
            
            # Chỉ chuyển sang CPU ở bước cuối cùng
            self.sum_trees[env_i].update_ids(indices_slice.detach().cpu(), prob_slice.detach().cpu())
    ...
    def update_cum_rewards(self, get_cumulative_rewards):
        """
        Cập nhật phần thưởng tích lũy của các mẫu mới trong buffer.
        """
...
class SumTree:
    ...
    def update_ids(self, data_ids: TEN, prob: TEN = 10.):  # 10 is max_prob
        # Lọc và tách các indices vượt quá giới hạn
        valid_mask = (data_ids >= 0) & (data_ids < self.buf_len)
        if not valid_mask.all():
            data_ids = data_ids[valid_mask]
