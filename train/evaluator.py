'''
1. File này giúp đánh giá hiệu suất của agent trong quá trình huấn luyện thông qua việc chạy environment đánh giá.
2. Khi agent đạt được hiệu suất tốt hơn, Evaluator lưu mô hình actor lại để sử dụng sau này.
3. Tạo biểu đồ hiển thị tiến trình học tập, giúp hiểu rõ hơn về quá trình huấn luyện.
4. Hỗ trợ cả môi trường đơn và môi trường vector hóa (đa luồng)
'''
...
class Evaluator:
    def __init__(self, cwd: str, env, args: Config, if_tensorboard: bool = False):
        ...
        self.stock_code = getattr(env, 'stock_code', 'Unknown')
        self.stock_type = getattr(env, 'stock_type', 'Unknown')
    ...
    def evaluate_and_save(self, actor: th.nn, steps: int, exp_r: float, logging_tuple: tuple):
        '''
        1. Cập nhật tổng số bước huấn luyện đã diễn ra
        2. Kiểm tra xem đã đến thời điểm đánh giá chưa
        3. Chạy agent trong môi trường đánh giá để thu thập số liệu về phần thưởng và số bước
        4. Tính toán các giá trị thống kê (trung bình, độ lệch chuẩn)
        5. Ghi lại thông tin huấn luyện và hiển thị lên terminal
        5. Lưu mô hình actor nếu đạt được hiệu suất tốt hơn
        '''
        ...
        if if_save:  # save checkpoint with the highest episode return
            best_actor_path = f"{self.cwd}/actor.pth"
            th.save(actor, best_actor_path)
            print(f"Save the best model with reward: {avg_r:.2f} in {best_actor_path}")

            # Still save the version with the highest score
            if not self.if_over_write:
                actor_path = f"{self.cwd}/actor_{self.total_step:010d}_.pt"
                th.save(actor, actor_path)

        elif self.save_counter == self.save_gap:
            self.save_counter = 0
            if self.if_over_write:
                actor_path = f"{self.cwd}/actor_latest.pt"
            else:
                actor_path = f"{self.cwd}/actor__{self.total_step:012}.pt"
                th.save(actor, actor_path)

        #Save curve learning
        if if_save or (actor_path is not None):
            self.save_training_curve_jpg()
    ...
    def save_training_curve_jpg(self):
        ...
        stock_code = getattr(self.env, "stock_code", "Unknown")
        fig_title = f"{self.stock_code}_{self.stock_type}_step{int(total_step)}_maxR{self.max_r:.3f}"

        save_name = f"{stock_code}_LearningCurve.jpg"
        draw_learning_curve(recorder=recorder, fig_title=fig_title, save_path=f"{self.cwd}/{save_name}")
...
def draw_learning_curve(recorder: np.ndarray = None,
    ...
    '''plot subplots'''
    # Tạo figure và axes mới mỗi lần gọi hàm
    fig, axs = plt.subplots(2, 1, figsize=(15, 14), sharex=True)
