"""
Script chạy thực nghiệm cho cổ phiếu Mỹ (S&P 500)
...
# ==================== DANH SÁCH MÃ CỔ PHIẾU MỸ (S&P 500 Selection) ====================
# Top 100 stocks from S&P 500 by market cap
US_STOCKS = [
    # Technology
    "AAPL", "MSFT", "GOOGL", "GOOG", "META", "NVDA", "TSLA", "AMD", "INTC", "AVGO",
    "ORCL", "CRM", "ADBE", "CSCO", "ACN", "IBM", "TXN", "QCOM", "NOW", "INTU",
    "AMAT", "ADI", "LRCX", "MU", "SNPS", "KLAC", "CDNS", "MRVL", "PANW", "CRWD",
    
    # Finance
    "JPM", "V", "MA", "BAC", "WFC", "GS", "MS", "C", "AXP", "BLK",
    "SCHW", "SPGI", "CME", "ICE", "MCO", "AON", "MMC", "PNC", "TFC", "USB",
    
    # Healthcare
    "UNH", "JNJ", "PFE", "ABBV", "LLY", "MRK", "TMO", "ABT", "DHR", "BMY",
    "AMGN", "GILD", "ISRG", "CVS", "ELV", "MDT", "SYK", "ZTS", "VRTX", "REGN",
    
    # Consumer
    "AMZN", "HD", "NKE", "MCD", "SBUX", "TGT", "LOW", "COST", "WMT", "PG",
    "KO", "PEP", "PM", "MO", "CL", "EL", "MDLZ", "GIS", "KHC", "K",
    
    # Industrial & Energy
    "XOM", "CVX", "COP", "SLB", "EOG", "PXD", "OXY", "VLO", "MPC", "PSX",
    "CAT", "DE", "HON", "UPS", "BA", "LMT", "RTX", "GE", "MMM", "EMR"
]


def download_stock_data(stock_code: str, output_dir: str) -> bool:
    """Tải dữ liệu cổ phiếu Mỹ từ Yahoo Finance"""
    if yf is None:
    ...
    try:
        ...
        # Đảm bảo Date là datetime và remove timezone
...
def save_results(results: list, output_path: str):
    ...
    if success_results:
        ...
        print(f"\n{'='*60}")
        print(f"US EXPERIMENT SUMMARY (S&P 500)")
        print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description='Run US Stock Experiment')
    parser.add_argument('--mode', type=str, default='train',
                       choices=['all', 'train', 'test'],
                       help='Mode: all (download+train+test), train (train+test), test (test only)')
    parser.add_argument('--data_dir', type=str, default='./data/us',
                       help='Directory to store/read stock data')
    parser.add_argument('--output', type=str, default='./results/us_results.csv',
    ...
    print(f"\n{'#'*60}")
    print(f"🇺🇸 US STOCK EXPERIMENT (S&P 500)")
    print(f"{'#'*60}")
